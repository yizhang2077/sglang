        ...
        **kwargs,
        ...
        if is_target_verify:
            ...
            intermediate_q_state_cache[row, col] = query.view(
                -1, extend_len, layer.num_q_heads, layer.head_q_dim
            )
            intermediate_k_state_cache[row, col] = key.view(
                -1, extend_len, layer.num_k_heads, layer.head_k_dim
            )
            intermediate_v_state_cache[row, col] = value.view(
                -1, extend_len, layer.num_v_heads, layer.head_v_dim
            )
            intermediate_beta_state_cache[row, col] = beta.view(
                -1, extend_len, layer.num_v_heads
            )
            ...
            query_start_loc = torch.arange(
                0,
                (FLA_CHUNK_SIZE + extend_len) * batch_size + 1,
                step=FLA_CHUNK_SIZE + extend_len,
                dtype=torch.int32,
                device=query.device,
            )

            core_attn_out = chunk_gated_delta_rule(
                q=intermediate_q_state_cache[cache_indices]
                .view(1, -1, layer.num_q_heads, layer.head_q_dim)
                .contiguous(),
                k=intermediate_k_state_cache[cache_indices]
                .view(1, -1, layer.num_k_heads, layer.head_k_dim)
                .contiguous(),
                v=intermediate_v_state_cache[cache_indices]
                .view(1, -1, layer.num_v_heads, layer.head_v_dim)
                .contiguous(),
                g=intermediate_g_state_cache[cache_indices]
                .view(1, -1, layer.num_v_heads)
                .contiguous(),
                beta=intermediate_beta_state_cache[cache_indices]
                .view(1, -1, layer.num_v_heads)
                .contiguous(),
