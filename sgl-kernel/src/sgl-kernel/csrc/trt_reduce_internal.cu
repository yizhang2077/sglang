...
__inline__ __device__ void block_barrier(uint32_t** signals,
                                         uint32_t const flag,
                                         size_t const local_rank,
                                         size_t const world_size,
                                         int const tidx,
...
static __global__ void twoShotAllReduceKernel(AllReduceParams params) {
  ...
  const size_t chunk_start = bidx * params.elts_per_block + tidx * NUM_ELTS;
  size_t chunk_end = std::min(chunk_start + params.elts_per_block, params.elts_per_rank);

  // The source pointers. Distributed round-robin for the different warps.
  T *buffers[RANKS_PER_NODE];
  // The destination ranks for round-robin gathering
  size_t ranks[RANKS_PER_NODE];
...
#pragma unroll
  ...
  // Each block accumulates the values from the different GPUs on the same node.
  for (size_t local_offset = chunk_start; local_offset < chunk_end; local_offset += blockDim.x * NUM_ELTS) {
    auto responsible_block_offset = local_offset + params.rank_offset;
